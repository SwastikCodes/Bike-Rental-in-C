...
void add()
{
	...
	if((f = fopen("add.txt","r+"))!=NULL)
	{
		...
		printf("\n Enter bike number\t: ");
		scanf(" i%",&bikenumber);
		while(fscanf(f,"%i %s %s %s %s %s %i\n", &s.bikenumber, s.name, s.address, s.phonenumber, s.aadhar, s.email, &s.period)!=EOF)
	    {
		...
		do
		{
			...
			printf("\nEnter Aadhar no.\t: ");		
			scanf("%s",s.aadhar);
			if(strlen(s.aadhar)>12 || strlen(s.aadhar)<2)
			{
				printf("\nInvalid!!\n\tThe maximum aadhar can be upto 15 and and more than 1.\n");
				goto X;
			}
...
void delete1()
{
	...
	else
	{
		...
		while(fscanf(f,"%i %s %s %s %s %s %i\n", &s.bikenumber, s.name, s.address, s.phonenumber, s.aadhar, s.email, &s.period)!=EOF)
		{
			...
			{
				fprintf(t,"%i %s %s %s %s %s %i\n", s.bikenumber, s.name, s.address, s.phonenumber, s.aadhar, s.email, s.period);
			}
			else
			{
				i=1;
